...
die "Usage $0 query_species query_chromosome query_start query_end target_species_set_name [coords|align|debug] [method]\n" if scalar(@ARGV) < 6;
...
my @method_links = $ARGV[6] ? ($ARGV[6]) : qw(EPO_LOW_COVERAGE EPO PECAN);
my $methodLinkSpeciesSet;
foreach my $ml (@method_links) {
    $methodLinkSpeciesSet = $method_link_species_set_adaptor->fetch_by_method_link_type_species_set_name($ml, $ARGV[4]);
    last if $methodLinkSpeciesSet;
}
die "Could not find a multiple alignment for $ARGV[4] (tried @method_links)\n" unless $methodLinkSpeciesSet;
...
foreach my $genomic_align_block( @{ $all_genomic_align_blocks }) {
    my $restricted_gab = $genomic_align_block->restrict_between_reference_positions($query_start, $query_end, undef, 'skip_empty_GenomicAligns');
